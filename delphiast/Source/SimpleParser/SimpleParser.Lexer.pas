...
type
  ...
  private
    ...
    function GetTokenLen: PtrInt; inline;
    ...
    function GetRunPos: PtrInt;
    procedure SetRunPos(const Value: PtrInt);
  ...
  public
    ...
    property RunPos: PtrInt read GetRunPos write SetRunPos;
    property Token: string read GetToken;
    property TokenLen: PtrInt read GetTokenLen;
    property TokenPos: PtrInt read FTokenPos;
...
function TmwBasePasLex.GetRunPos: PtrInt;
begin
  Result := FBuffer.Run;
end;
...
begin
  if Assigned(FBuffer.Buf) and not FBuffer.SharedBuffer then
    FreeMem(FBuffer.Buf);
  BufferSize := (Length(NewValue) + 1) * SizeOf(Char);

  GetMem(FBuffer.Buf, BufferSize);
  FillChar(FBuffer.Buf^, BufferSize, 0);
  if Length(NewValue) > 0 then
    Move(NewValue[1], FBuffer.Buf^, Length(NewValue) * SizeOf(Char));

  Init;
  Next;
end;

procedure TmwBasePasLex.SetRunPos(const Value: PtrInt);
begin
  FBuffer.Run := Value;
  Next;
end;
...
function TmwBasePasLex.GetTokenLen: PtrInt;
begin
